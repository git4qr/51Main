...
static Int32 uart_port_open(struct _port_handle *Hdl, void *openParams);
static Int32 uart_port_close(struct _port_handle *Hdl);
static Int32 uart_port_set(uart_port_handle *port);
static Int32 uart_port_send(struct _port_handle *Hdl, UInt8 *data, UInt32 len);
static Int32 uart_port_recv(struct _port_handle *Hdl, UInt8 *buffer, UInt32 size);
...
Int32 uart_port_create(port_handle **ppHdl)
{
	...
	memset(uartPort, 0, sizeof(uart_port_handle));

	uartPort->Hdl.context = uartPort;
...
{
	uart_port_handle *uartPort = (uart_port_handle *)pHdl->context;
