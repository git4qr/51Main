...
void usd_MSGAPI_ExtInpuCtrl_TrkSearch(long p)
{
	...
	//   m_selectPara.SecAcqStat=(unsigned char)m_CurrStat.m_SecTrkStat;
		//m_selectPara.ImgPixelX=sThis->m_jos->WinX;
	//	m_selectPara.ImgPixelY=sThis->m_jos->WinY;
		//sThis->m_ipc->ipcSecTrkCtrl(&m_selectPara);
		//fprintf(stdout, "WINX = %d\n", m_selectPara.ImgPixelX);
   	//	fprintf(stdout, "WINY = %d\n", m_selectPara.ImgPixelY);
}
