...
/*    boresightPosTab                                                   */
/************************************************************************/
__INLINE SensorComp_BoresightPosTab* boresightPosTab_alloc(int size)
{
...
__INLINE void boresightPosTab_free(SensorComp_BoresightPosTab* tab)
{
	assert(tab != NULL);
...
__INLINE void boresightPosTab_BuildDef(SensorComp_BoresightPosTab* tab, int x, int y,
...
__INLINE void boresightPosTab_Build(SensorComp_BoresightPosTab* tab, 
...
__INLINE SensorComp_BoresightPosTab* boresightPosTab_copy(SensorComp_BoresightPosTab* dest, 
...
__INLINE int boresightPosTab_find(SensorComp_BoresightPosTab* tab, 
									float fov)
{
	int i = 0;
...
__INLINE int boresightPosTab_insert(SensorComp_BoresightPosTab* tab,
...
/*    SensorComp                                                        */
/************************************************************************/
/* 内部函数：根据邻近靶心位置计算插值，约束条件邻近靶心（pObj->iCurPos）已确定 */
__INLINE void SensorComp_GetBoresightPos_(SensorComp_Obj *pObj, float fFov, int *pX, int *pY)
{
	int x = 0, y = 0;
...
/* 设置靶心位置，只在校轴模式下调用，返回：0 成功，其他 失败  */
