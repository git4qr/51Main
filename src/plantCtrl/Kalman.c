...
HKalman KalmanInit()
{
	...
	pKalamObj->deltat = 0.04; // 视频采样时间间隔
...
void KalmanInitParam(HKalman hKalman, float theta, float delta_x, float delta_y, double DeltaT)
{
	...
	hKalman->process_noise_cov[1*hKalman->DP+1] = 1E-5;//0.00000001;//100.0;  /* 协方差都为100.0，且相互独立 */
	hKalman->process_noise_cov[3*hKalman->DP+3] = 1E-5;//0.00000001;//100.0;  /* 协方差都为100.0，且相互独立 */
	hKalman->process_noise_cov[5*hKalman->DP+5] = 1E-5;//0.00000001;//100.0;  /* 协方差都为100.0，且相互独立 */
	...
	hKalman->measurement_noise_cov[0*hKalman->MP+0] = 0.25;  /* 协方差都为0.001，且相互独立 */
	hKalman->measurement_noise_cov[1*hKalman->MP+1] = 0.25;  /* 协方差都为0.001，且相互独立 */
	hKalman->measurement_noise_cov[2*hKalman->MP+2] = 0.25;  /* 协方差都为0.001，且相互独立 */
	...
	for ( y = 0; y < hKalman->DP; y++ ){
		hKalman->error_cov_post[y*hKalman->DP+y] = 1.0;  /* 对角初始协方差都为1，且相互独立 */
	}
	...
	for ( y = 0; y < hKalman->DP; y++ ){
		hKalman->transition_matrix[y*hKalman->DP+y] = 1.0;  /* 对角为1 */
	}
	...
	/* 初始化thelta, thelta_v, x, x_v, y, y_v等状态量 */
    // 状态量，排列顺序：thelta, thelta_v,  x, y, x_v, y_v
